	...
	while (leave == 0)
	{
		...
		switch(opcode)
		{
			...
			else if (imm16 >= 17000 && imm16 <= 17100)
			{
				...
				next_script = vm_enter_code(imm16, &played);
			...
			#if HOTA_DIAG
			{
				int sp = (int)get_variable(6);
				int ok = (sp >= 0 && sp < 64);

				fprintf(stderr, "dth a%d rm%d pc%04x v2=%d v6=%d y=%d x=%d\n",
					imm8, current_room, (unsigned int)pc,
					(int)get_variable(2), sp,
					ok ? sprites[sp].y : -1,
					ok ? sprites[sp].x : -1);
			}
			#endif
			...
			if (death_played)
			{
				if (death_site_is_real(current_room, pc - 2))
				{
					next_script = 7;
					leave = 1;
				}
			...
			if (var1 == VM_VAR_CODE_SEEN)
			{
#if HOTA_DIAG
				fprintf(stderr, "acs rm%d e%d skip%d dt%u ld%u\n",
					current_room, (int)get_variable(220), access_code_skip,
					platform_ticks() - access_code_skip_at,
					access_code_load_seq);
#endif

				if (access_code_skip == ACCESS_CODE_SKIP_LIVE)
				{
					access_code_skip = 0;
