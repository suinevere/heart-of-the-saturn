...
static void test_header_roundtrip(void)
{
    ...
    savedata_write_header(buf, 23, 2, 0x11223344UL, 1, 4321);
    expect_int("header parses", savedata_read_header(buf, &ver, &room, &entry,
                                                     &date, &flags,
                                                     &payloadLen), 1);
    expect_int("version", (int)ver, SAVE_FORMAT_VERSION);
    expect_int("room", (int)room, 23);
    expect_int("entry", (int)entry, 2);
...
static void test_a_state_header_still_writes_zero_at_byte_seven(void)
{
    unsigned char buf[SAVE_HEADER_SIZE];

    savedata_write_header(buf, 3, 0, 0UL, 0, 16);
    expect_int("byte 7 is what a pre-checkpoint save wrote there",
               (int)buf[7], 0);
}

static void test_a_checkpoint_header_is_a_header_and_nothing_else(void)
{
    unsigned char buf[SAVE_HEADER_SIZE];
    unsigned short ver = 0, room = 0, payloadLen = 9;
    unsigned char flags = 0, entry = 0;
    unsigned long date = 0;

    savedata_write_header(buf, 6, 3, 0UL, SAVE_FLAG_CHECKPOINT, 0);
    expect_int("it parses", savedata_read_header(buf, &ver, &room, &entry,
                                                 &date, &flags, &payloadLen), 1);
    expect_int("carrying the flag", (int)(flags & SAVE_FLAG_CHECKPOINT),
               SAVE_FLAG_CHECKPOINT);
    expect_int("the room", (int)room, 6);
    expect_int("the entry", (int)entry, 3);
    expect_int("and no payload", (int)payloadLen, 0);
}
...
static void test_header_rejects_bad_magic(void)
{
    ...
    savedata_write_header(buf, 1, 0, 2, 0, 10);
    buf[2] = 'X';
    expect_int("bad magic rejected",
               savedata_read_header(buf, &ver, &room, &entry, &date, &flags,
                                    &payloadLen), 0);
...
static void test_probe_ok_slot(void)
{
    ...
    stub_bup_reset();
    savedata_write_header(file, 42, 0, 0x00010203UL, 0, 4);
    ...
    expect_int("and it is a state save", (int)(info.flags & SAVE_FLAG_CHECKPOINT), 0);
}

static void test_probe_reports_a_checkpoint_slot(void)
{
    unsigned char scratch[SAVE_MAX_BYTES];
    unsigned char file[SAVE_HEADER_SIZE];
    SlotInfo info;

    stub_bup_reset();
    savedata_write_header(file, 4, 3, 0x00010203UL, SAVE_FLAG_CHECKPOINT, 0);
    stub_bup_place(SAT_BUP_INTERNAL, "HOTASAVE1", file, (int)sizeof(file));

    expect_int("a header-only slot is loadable",
               (int)savedata_probe(SAT_BUP_INTERNAL, 0, &info, scratch,
                                   SAVE_MAX_BYTES),
               (int)SLOT_OK);
    expect_int("and says so", (int)(info.flags & SAVE_FLAG_CHECKPOINT),
               SAVE_FLAG_CHECKPOINT);
    expect_int("with its room", (int)info.roomId, 4);
    expect_int("and its entry", (int)info.entry, 3);
}
...
static void test_probe_damaged_slot(void)
{
    ...
    stub_bup_reset();
    savedata_write_header(file, 42, 0, 1, 0, 4);
    file[0] = 'Z';
    stub_bup_place(SAT_BUP_INTERNAL, "HOTASAVE1", file, (int)sizeof(file));
...
static void test_probe_newer_version(void)
{
    unsigned char scratch[SAVE_MAX_BYTES];
    unsigned char file[SAVE_HEADER_SIZE + 4];
    SlotInfo info;
    ...
    stub_bup_reset();
    savedata_write_header(file, 42, 0, 1, 0, 4);
    file[4] = (unsigned char)(((SAVE_FORMAT_VERSION + 1) >> 8) & 0xFF);
    file[5] = (unsigned char)((SAVE_FORMAT_VERSION + 1) & 0xFF);
    stub_bup_place(SAT_BUP_INTERNAL, "HOTASAVE1", file, (int)sizeof(file));

    expect_int("a version we do not know is not OK either",
               (int)savedata_probe(SAT_BUP_INTERNAL, 0, &info, scratch,
                                   SAVE_MAX_BYTES),
               (int)SLOT_OLD_VERSION);
}

static void test_probe_rejects_payload_longer_than_the_file(void)
{
    unsigned char scratch[SAVE_MAX_BYTES];
    unsigned char file[SAVE_HEADER_SIZE + 4];
    SlotInfo info;
    ...
    stub_bup_reset();
    savedata_write_header(file, 42, 0, 1, 0, 64);
    memset(file + SAVE_HEADER_SIZE, 0, 4);
    stub_bup_place(SAT_BUP_INTERNAL, "HOTASAVE1", file, (int)sizeof(file));

    expect_int("64 bytes of payload claimed, 4 present",
               (int)savedata_probe(SAT_BUP_INTERNAL, 0, &info, scratch,
                                   SAVE_MAX_BYTES),
               (int)SLOT_DAMAGED);
}

static void test_probe_rejects_a_state_save_with_no_payload(void)
{
    unsigned char scratch[SAVE_MAX_BYTES];
    unsigned char file[SAVE_HEADER_SIZE + 4];
    SlotInfo info;
    ...
    stub_bup_reset();
    savedata_write_header(file, 42, 0, 1, 0, 0);
    memset(file + SAVE_HEADER_SIZE, 0, 4);
    stub_bup_place(SAT_BUP_INTERNAL, "HOTASAVE1", file, (int)sizeof(file));

    expect_int("a state save with an empty payload is damaged",
               (int)savedata_probe(SAT_BUP_INTERNAL, 0, &info, scratch,
                                   SAVE_MAX_BYTES),
               (int)SLOT_DAMAGED);
}
...
int main(void)
{
    ...
    test_a_state_header_still_writes_zero_at_byte_seven();
    test_a_checkpoint_header_is_a_header_and_nothing_else();
    test_header_rejects_bad_magic();
    test_probe_empty_slot();
    test_probe_ok_slot();
    test_probe_reports_a_checkpoint_slot();
    test_probe_damaged_slot();
    test_probe_old_version();
    test_probe_newer_version();
    test_probe_rejects_payload_longer_than_the_file();
    test_probe_rejects_a_state_save_with_no_payload();
